...
class SLRExerciseAnalyzer:
    ...
    def run(self):
        ...
        if not cap.isOpened():
            print("[ERROR] Could not open webcam. Please check if the camera is connected and not used by another application.")
            logger.error("Could not open webcam. Please check if the camera is connected and not used by another application.")
            return
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
