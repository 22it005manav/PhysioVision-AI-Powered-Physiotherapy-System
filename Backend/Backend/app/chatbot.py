...
# Try to import Mistral from different locations for compatibility
try:
    ...
    try:
        ...
        print("Error: Could not import Mistral from mistralai. Please install mistralai package.")
        Mistral = None
...
class RAG:
    def __init__(self, api_key = "QsVtSj8rL1TZVL0l2usBvNPk0dF7bOMZ",  file_path=None):
        ...
        # Use current directory if no file path specified
        if file_path is None:
            file_path = os.path.join(os.path.dirname(__file__), "Database")
        self.file_path = file_path
        self.model =   "mistral-large-latest"
        
        # Initialize Mistral client with fallback
    ...
    def load_documents(self):
        try:
            self.patient_embeddings = np.load(os.path.join(self.file_path, "patient_embeddings.npy"))
            self.nutrition_embeddings = np.load(os.path.join(self.file_path, "nutrition_embeddings.npy"))
            self.patient_npy = np.load(os.path.join(self.file_path, "patient_documents.npy"), allow_pickle = True)
            self.nutrition_npy = np.load(os.path.join(self.file_path, "nutrition_documents.npy") , allow_pickle = True)
            ...
            print(f"Warning: Database files not found at {self.file_path}. RAG will run in demo mode. Error: {e}")
            # Initialize empty lists for demo mode
            self.patient_embeddings = np.array([])
            self.nutrition_embeddings = np.array([])
            self.patient_documents = []
            self.nutrition_documents = []
    
    
    def concatinate_userdocument(self, query, user_document):
        if self.client is None:
            print("Warning: Mistral client not initialized. Returning original query.")
            return query
